...
<div class="highlight">
<?php 
$query="SELECT DISTINCT user_id FROM support WHERE  sender!=3 ORDER BY rec_num asc";
$results=$db->get_results($query); ?>
...
<?php else: ?>
<li class="list-group-item">No old messages</li>
<?php endif ?>  
</div>
<div class="older">
<a href="messages" class="list-group-item">NEWER</a>
</div>
</div>
...
<div class="highlight">
<?php 
$query="SELECT DISTINCT user_id FROM support WHERE status=1 and sender!=3 ORDER BY rec_num asc";
$results=$db->get_results($query); ?>
<?php if ($db->num_rows>0): ?>

<?php foreach ($results as $result) {
$query="SELECT count(message) FROM support where user_id=$result->user_id AND status=1 AND sender !=3" ;
$result2=$db->get_var($query);
?>
<form action="" method="GET" id="prevew_message">
<!-- <li class="list-group-item">User Id <?php #secho $result->user_id."   ". "(".$result2.")"; ?></li> -->
